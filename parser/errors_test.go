...
func TestParseFailures(t *testing.T) {
	tests := []struct {
		...
		{"fn test(,) {}", "1:9: Expected IDENT or RPAREN got COMMA"},
		{"fn test(bar i = 1) {}", "1:13: Expected RPAREN or COMMA got i"},
		{"fn test(int) => {}", "1:17: Expected function return type got LBRACE({)"},
		{"fn test(foo : float foo) {}", "1:21: Expected RPAREN or COMMA got foo"},
		...
		{"fn ( {}", "1:6: Expected IDENT or RPAREN got LBRACE"},
		...
		{"fn foobar() {  fn foobar(i:int;) {} }", "1:31: Expected RPAREN or COMMA got SEMICOLON"},
		// Member expressions
		...
		{"fn foobar() { var foo = 1 + 1 * }", "1:33: Expected expression got RBRACE(})"},
		// UnaryExpression
		...
		{"macro M { () : }", "1:17: Expected LPAREN, LBRACE or LBRACK got EOF"},
